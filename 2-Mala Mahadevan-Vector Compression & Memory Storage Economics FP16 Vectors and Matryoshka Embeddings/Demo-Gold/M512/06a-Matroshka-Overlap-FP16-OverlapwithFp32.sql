...
DECLARE @Matryoshka512_FP16Query VECTOR(512, FLOAT16);

SELECT @FP32Query = embedding
FROM dbo.ArticlesEmbeddings_FP32
WHERE hn_id = @QueryHnId;

SELECT @Matryoshka512_FP16Query = embedding
FROM dbo.ArticlesEmbeddings_Matryoshka512_FP16
WHERE hn_id = @QueryHnId;

IF @FP32Query IS NULL OR @Matryoshka512_FP16Query IS NULL
    THROW 50001, 'Query embedding is missing from FP32 or Matryoshka512_FP16.', 1;

DROP TABLE IF EXISTS #FP32Results;
DROP TABLE IF EXISTS #Matryoshka512_FP16Results;
...
/* Matryoshka512_FP16 Top 10 */

SELECT TOP (@TopK)
    ROW_NUMBER() OVER
    (
        ORDER BY
            VECTOR_DISTANCE('cosine', @Matryoshka512_FP16Query, e.embedding),
            e.hn_id
    ) AS neighbor_rank,
...
INTO #Matryoshka512_FP16Results
...
SELECT
    ...
    f16.title AS Matryoshka512_FP16_neighbor,
    f16.cosine_distance AS Matryoshka512_FP16_distance,
...
FULL JOIN #Matryoshka512_FP16Results AS f16
...
FROM #FP32Results AS f32
JOIN #Matryoshka512_FP16Results AS f16
