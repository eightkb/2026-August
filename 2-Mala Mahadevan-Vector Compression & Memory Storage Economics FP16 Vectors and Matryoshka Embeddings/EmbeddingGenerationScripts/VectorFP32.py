# 01_generate_fp32_embeddings_key_tokens.py
...
TARGET_TABLE = "dbo.ArticlesEmbeddings_FP32"
...
print("Target table ready.", flush=True)
...
def count_tokens(text_value) -> int:
    # Approximate token count for demo purposes.
    # Good enough for storage/filtering demos.
...
def insert_rows(rows):
    ...
    sql = text(f"""
        ...
        SELECT
            :hn_id,
            :n_tokens,
            CAST(:embedding AS VECTOR(768, FLOAT32))
        WHERE NOT EXISTS
        (
            SELECT 1
            FROM {TARGET_TABLE}
...
while True:
    ...
    print(
        f"Generating embeddings for {len(texts)} rows. "
    ...
    for idx, row in df.iterrows():
        rows.append({
            ...
            "embedding": vector_to_json(embeddings[idx])
        })
    ...
    print(f"Inserted this run: {total_inserted:,}", flush=True)

print(f"Done. Inserted this run: {total_inserted:,}", flush=True)