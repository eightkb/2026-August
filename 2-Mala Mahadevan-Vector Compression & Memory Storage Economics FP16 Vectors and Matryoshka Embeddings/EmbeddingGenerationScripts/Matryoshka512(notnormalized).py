# 03_generate_matryoshka512_fp16_embeddings_key_tokens.py
...
TARGET_TABLE = "dbo.ArticlesEmbeddings_Matryoshka512_FP16"
...
BEGIN
    ...
    (
        ...
        embedding VECTOR({MATRYOSHKA_DIM}, FLOAT16) NOT NULL
    );
END;
"""
...
print("Matryoshka FP16 target table ready.", flush=True)
...
if MATRYOSHKA_DIM > embedding_dim:
    raise ValueError(
        f"MATRYOSHKA_DIM {MATRYOSHKA_DIM} cannot exceed model dimension {embedding_dim}"
    )
...
def insert_rows(rows):
    ...
    sql = text(f"""
        ...
        SELECT
            :hn_id,
            :n_tokens,
            CAST(:embedding AS VECTOR({MATRYOSHKA_DIM}, FLOAT16))
        WHERE NOT EXISTS
        (
            SELECT 1
            FROM {TARGET_TABLE}
...
while True:
    ...
    print(
        f"Generating Matryoshka-{MATRYOSHKA_DIM} FP16 embeddings for {len(texts)} rows. "
        f"hn_id range {int(df['hn_id'].min())} - {int(df['hn_id'].max())}",
        flush=True
    )

    embeddings_fp32_full = model.encode(
        texts,
    ...
    # Matryoshka step:
    # keep the first N dimensions, then normalize again.
    embeddings_mrl = embeddings_fp32_full[:, :MATRYOSHKA_DIM]
    embeddings_mrl = l2_normalize(embeddings_mrl).astype(np.float32)

    # Compression step:
    # store the truncated Matryoshka vector as FLOAT16.
    embeddings_mrl_fp16 = embeddings_mrl.astype(np.float16)
    ...
    for idx, row in df.iterrows():
        rows.append({
            ...
            "embedding": vector_to_json(embeddings_mrl_fp16[idx])
        })
    ...
    print(
        f"Inserted Matryoshka-{MATRYOSHKA_DIM} FP16 this run: {total_inserted:,}",
        flush=True
    )

print(
    f"Done. Inserted Matryoshka-{MATRYOSHKA_DIM} FP16 this run: {total_inserted:,}",
    flush=True
)